...
fun BodyResolveComponents.computeRepresentativeTypeForBareType(type: ConeClassLikeType, originalType: ConeKotlinType): ConeKotlinType? {
    ...
    return expandedCastType.withArguments(newArguments.toTypedArray()).takeIf {
        it.isSubtypeOf(originalType.withNullability(ConeNullability.NULLABLE, session.typeContext), session)
    }
}
...
private fun ConeTypeProjection.removeQuestionMark(typeContext: ConeTypeContext): ConeTypeProjection {
    ...
    return when (kind) {
        ProjectionKind.INVARIANT -> notNullableType
        ProjectionKind.IN -> ConeKotlinTypeProjectionIn(notNullableType)
        ProjectionKind.OUT -> ConeKotlinTypeProjectionOut(notNullableType)
